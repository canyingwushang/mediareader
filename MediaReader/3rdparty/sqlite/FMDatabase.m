    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            ...
            if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                DSLog(@"%s:%d", __FUNCTION__, __LINE__);
                DSLog(@"Database busy, unable to close");
                return NO;
            }
        }
        else if (SQLITE_OK != rc) {
            DSLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        DSLog(@"error on rekey: %d", rc);
        DSLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)compainAboutInUse {
    DSLog(@"The FMDatabase %@ is currently in use.", self);
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc) {
                ...
                if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                    DSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    DSLog(@"Database busy");
                ...
                if (logsErrors) {
                    DSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    DSLog(@"DB Query: %@", sql);
                    if (crashOnErrors) {
...
//#endif
    ...
    if (idx != queryCount) {
        DSLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return nil;
    }
...
//#endif
    ...
    if (idx != queryCount) {
        DSLog(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            ...
            DSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(db));
            DSLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            DSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(db));
            DSLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            DSLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(db));
            DSLog(@"DB Query: %@", sql);
        }
