...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        DSLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            ...
            if ([parentDB busyRetryTimeout] && (numberOfRetries++ > [parentDB busyRetryTimeout])) {
                ...
                DSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [parentDB databasePath]);
                DSLog(@"Database busy");
                break;
            }
        }
            ...
            DSLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            DSLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    DSLog(@"Warning: I could not find the column named '%@'.", columnName);
