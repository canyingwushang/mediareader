...
- (void)setMode:(KSProgressMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == KSProgressModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(KSRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == KSProgressModeDeterminate) {
        self.indicator = [[[KSRoundProgressView alloc] init] autorelease];
    }
    else if (mode == KSProgressModeCustomView && self.customView != nil){
...
+ (KSProgress *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	KSProgress *hud = [[KSProgress alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[KSProgress class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		KSProgress *HUD = (KSProgress *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"KSProgressViewIsNillException" 
					format:@"The view used in the KSProgress initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = KSProgressAnimationFade;
        self.mode = KSProgressModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == KSProgressAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == KSProgressAnimationZoom) {
...
@implementation KSRoundProgressView
